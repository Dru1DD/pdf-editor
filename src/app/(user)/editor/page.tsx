...
import { Upload, Save, StickyNote, X, Pencil, Type, MousePointer2 } from 'lucide-react';
...
export default function EditorPage() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const pdfCanvas = containerRef.current?.querySelector('canvas') as HTMLCanvasElement;
          ...
          >
            <Worker workerUrl="https://unpkg.com/pdfjs-dist@3.11.174/build/pdf.worker.min.js">
              <div
                ...
                style={{
                  cursor: currentTool === 'pencil' ? 'crosshair' : currentTool === 'text' ? 'text' : 'pointer',
                }}
                onClick={currentTool === 'note' ? handleAddNote : currentTool === 'text' ? handleAddText : undefined}
              >
                  ...
                  >
                    {editingTextId === textEl.id ? (
                      <input
                        ...
                        onChange={(e) => {
                          setTextElements((prev) =>
                            prev.map((el) => (el.id === textEl.id ? { ...el, text: e.target.value } : el)),
                          );
                        }}
                      ...
                      <div className="relative group">
                        <span className="bg-indigo-500/80 px-2 py-1 rounded">{textEl.text}</span>
                        {currentTool === 'text' && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
      ...
      {fileUrl && (
        <div className="flex gap-4 mt-6 justify-center">
          ...
          >
            ...
            Export PDF ({notes.length + drawings.length + textElements.length} elements)
          </button>
        </div>
      )}
